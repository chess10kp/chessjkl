...
const Square = ({
  ...
  if (inCheck && piece) {
    //if A king is under check and this is a square with a piece
    if (piece.toUpperCase() == "K") {
      if (isWhite(piece) != turnToMove.current) {
        // if piece color is the same as the turnTomove
  ...
  const isSuggestionHighlight = suggestionCoords.current.find(
    (el) => el == `${rank}${file}`
  ) ? true : "";
  const classes = `square ${isBlackSquare ? "white" : "black"} ${
    isHighlight ? "highlight" : ""
  } ${isKeyHighlight ? "key-highlight" : ""} ${
    isCheckHighlight ? "check-highlight" : ""
  } ${isSuggestionHighlight ? (piece ? "suggestion-highlight-capture" : "suggestion-highlight" ): ""}`;
